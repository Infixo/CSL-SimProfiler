...
namespace SimProfiler
{
    ...
    public static class SimulationManager_Patches
    {
        ...
        public static bool SimulationManager_SimulationStep_Prefix(
        ...
        {
            // get names - well, they are the same but still...
            for (int i = 0; i < ___m_managers.m_size; i++)
                SimProfiler.SetName(i, ___m_managers.m_buffer[i].GetName());
            ...
            try
            {
                ...
                for (int i = num; i <= finalSimulationSpeed; i++)
                {
                    ...
                    {
                        if (i>0) SimProfiler.Begin(j);
                        ___m_managers.m_buffer[j].SimulationStep(i);
                        if (i>0) SimProfiler.End(j);
                    }
                    if (i != 0)
                    {
    ...
    {
        // protected override void VehicleManager::SimulationStepImpl(int subStep)
        ...
        public static bool VehicleManager_SimulationStepImpl_Prefix(VehicleManager __instance, int subStep)
        {
            ...
            // this part is called millions of times
